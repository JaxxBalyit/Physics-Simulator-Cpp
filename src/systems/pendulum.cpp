...
static PendulumState eulerStepPendulum(const PendulumState& s, double g, double L, double dt) {
  PendulumState ns;
...
static PendulumState rk4StepPendulum(const PendulumState& s, double g, double L, double dt) {
  auto deriv = [&](const PendulumState& st) {
    PendulumState d{st.omega, -(g/L)*std::sin(st.theta)};
    return d;
  };
...
void simulatePendulum(const std::string& integrator)
{
  ...
  PendulumState s = {M_PI/4, 0.0};
  std::vector<PendulumState> results;

  for (int i = 0; i < 2000; ++i) {
    results.push_back(s);
    s = (integrator == "euler") ?
      eulerStepPendulum(s, g, L, dt) :
      rk4StepPendulum(s, g, L, dt);
  }
